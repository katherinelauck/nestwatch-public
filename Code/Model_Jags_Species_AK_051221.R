...
nestwatch.sp <- function(d,
  ...
  model.jags<-function() {
    ...
    # mu2.tmaxxLUxSp~dnorm(0,.001)
    # mu2.tmaxxLUxSp2~dnorm(0,.001)
    # mu2.tmaxxLUxSp3~dnorm(0,.001)
    # mu2.tmaxxLUxSp4~dnorm(0,.001)
    ...
    # sig2.tmaxxLUxSp~dunif(0.001, 15)
    # tau2.tmaxxLUxSp<-1/sig2.tmaxxLUxSp^2
    # sig2.tmaxxLUxSp2~dunif(0.001, 15)
    # tau2.tmaxxLUxSp2<-1/sig2.tmaxxLUxSp2^2
    # sig2.tmaxxLUxSp3~dunif(0.001, 15)
    # tau2.tmaxxLUxSp3<-1/sig2.tmaxxLUxSp3^2
    # sig2.tmaxxLUxSp4~dunif(0.001, 15)
    # tau2.tmaxxLUxSp4<-1/sig2.tmaxxLUxSp4^2
    ...
    for(j in 1:38){
      ...
      # beta.tmax2xLUxSp[1,j]~dnorm(mu2.tmaxxLUxSp,tau2.tmaxxLUxSp)
      # beta.tmax2xLUxSp[2,j]~dnorm(mu2.tmaxxLUxSp2,tau2.tmaxxLUxSp2)
      # beta.tmax2xLUxSp[3,j]~dnorm(mu2.tmaxxLUxSp3,tau2.tmaxxLUxSp3)
      # beta.tmax2xLUxSp[4,j]~dnorm(mu2.tmaxxLUxSp4,tau2.tmaxxLUxSp4)
    }
    ...
    for(i in 1:nrows){
      ...
      logit(p[i])<-
       ...
       # beta.tmax2xLUxSp[NewLU1[i],Species[i]]*Tmax_std_gridmet_sq[i] +
  ...
  R2jags::jags(data=list(
   ...
   # Tmax_std_gridmet_sq=nest$Tmax_std_gridmet_sq,
...
setwd("~/nestwatch/results/Question 5/")

saveRDS(out,"sp_analysis_jags_051221.RDS")
