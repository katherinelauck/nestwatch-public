...
hex_res <- seq(7,30) # res 6 approximately corresponds to 200km between centers of neighboring hexes. See: http://cran.nexr.com/web/packages/dggridR/vignettes/dggridR.html and convert area to side length
...
model <- function(data,name) {
  library("lme4"); library(tidyverse);library(dplyr)
  gc()
  mod<-glmer(formula = at_least_one_success ~ Tmax_std_gridmet * NewLU1 + Tmax_std_gridmet_sq * NewLU1 + pcpbefore_raw_gridmet + NLCD_p_forest + NLCD_p_human + NLCD_p_ag + substrate_binary + laydate_scaled + (1 | species) + (1 | year) + (1 | Region/UnCoor), data = data, family = binomial(link = "logit"), control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e+06)))
  write_rds(mod,paste0("results/revisions/mainv1_10perhex",name,"quad.rds"))
  ...
  write_rds(data,paste0("results/revisions/mainv1_10perhex",name,"data.rds"))
  gc()
}



walk2(.x = l,.y = names(l),model)
