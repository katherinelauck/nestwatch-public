...
mod <- glmer(formula = at_least_one_success ~ Tmax_std_gridmet * 
             ...
             data = filter(nest,NewLU1 == "Ag"), family = binomial(link = "logit"), control = glmerControl(optimizer = "bobyqa", 
                                                                                    optCtrl = list(maxfun = 2e+05)))

write_rds(mod,"results/revisions/tavgnestpd_meanmax_gridmet_stdmaxlaydate3way_spslope_triple_ag.rds")
