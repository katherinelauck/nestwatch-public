...
mod <- read_rds('results/success~meanmax3way_gridmet_nomin_subfixed.rds')

options(na.action = "na.fail")
mods <- dredge(mod, beta = 'none', evaluate = FALSE, fixed = c("pcpbefore_raw_gridmet",'NLCD_p_forest','NLCD_p_human','NLCD_p_ag','substrate'))
options(na.action = "na.omit")

res <- eval(mods[[as.numeric(args[1])]])

save_rds(res, paste0("results/subfixed",args[1],'_out.rds'))
