...
# 8. Add in Modis Data 
#####################
processed$Year=substr(processed$laydate,1,4)
...
LatLonYear$YEAR_Update[which(is.na(as.numeric(LatLonYear$YEAR_Update)==TRUE))]=round(mean(na.omit(as.numeric(LatLonYear$Year)))) # Sites without a year are set to the average year (2012) 
...
########################
NLCD_2km <- read.csv("~/Google Drive/NestwatchProject/Data/NLCD/NLCD_2km_prop_clean.csv")
NLCD_2km <- NLCD_2km[,-c(1:3,5)]

processed$LOC_ID_year <- as.factor(paste(processed$loc_id,processed$year))

processed <- merge(processed,NLCD_2km,by="LOC_ID_year")

###still to merge other scales
...
processed$Region200=dgGEO_to_SEQNUM(dggs_200, processed$lon, processed$lat)$seqnum
processed$Region250=dgGEO_to_SEQNUM(dggs_250, processed$lon, processed$lat)$seqnum
...
sollman_spaut_failure <-sollman_spaut_failure[,c(2,5)]
names(sollman_spaut_failure)[2] <- c("sollman_spaut_failure")

sollman_spaut_success <-sollman_spaut_success[,c(2,5)]
names(sollman_spaut_success)[2] <- c("sollman_spaut_success")

processed <- merge(processed,sollman_spaut_failure,by="UnCoor",all.x=T)
processed <- merge(processed,sollman_spaut_success,by="UnCoor",all.x=T)
